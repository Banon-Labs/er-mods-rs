...
| scope | files | lines |
|---|---:|---:|
| all `experiments/**` | 80 | 50,697 |
| excluding `startup_hooks/**` | 44 | 25,311 |
| `startup_hooks/**` plus `startup_hooks.rs` | 36 | 25,386 |
| lifecycle S10 split | 5 | 2,304 |
| own-load S11 split | 5 | 2,909 |
| save redirect | 3 | 2,357 |

The `scripts/check-crate-extraction-roadmap.py` gate checks the 80 paths, line counts, and required caller edges below. A source file add/remove/line-count change must refresh this ledger in the same change.
