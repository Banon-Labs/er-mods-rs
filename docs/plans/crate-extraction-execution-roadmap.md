...
|---|---:|---|
| `startup_hooks/quit_menu/save_swap_profile_table.rs` | 566 | dead-counter census |
...
| `lifecycle/task_tick.rs` | 410 | dead-counter census |
| `input_block.rs` | 1,640 | dead-counter census |
| `startup_hooks/diagnostics/system_quit_ownership_repro.rs` | 1,308 | dead-counter census |
| `trace/menu_trace_hooks.rs` | 2,070 | stale-RVA call-site closure |
| `own_stepper/bootstrap_drive.rs` | 997 | stale-RVA call-site closure |
| `continue_load/slot_resolution.rs` | 857 | stale-RVA call-site closure |
| `trace/menu_constructor_capture.rs` | 1,402 | stale-RVA call-site closure |
| `startup_hooks.rs` | 115 | dead-counter census |
...
| S11 own-load | `own_load/request_wait_guard.rs` | the `CS::InGameStep::STEP_RequestWait` detour that keeps a switch-loaded world's session from being ended; D5 candidate | `lib_parts/dll_entry_parts` installs it; nothing else calls in |
...
| `continue_load/slot_resolution.rs` | 857 | product slot-resolution policy | D5 and R14 |
| `gating.rs` | 9 | product re-export facade | D1 |
| `gating/env_flags.rs` | 487 | product gate policy | D1 |
| `gating/runtime_modes.rs` | 60 | product runtime-mode policy | D1 |
...
| `input_block.rs` | 1,640 | product input ownership | `STAY` |
| `input_trace.rs` | 934 | product diagnostic | D4 |
| `lifecycle.rs` | 30 | S10 lifecycle facade | R20 |
| `lifecycle/hook_installers.rs` | 116 | product install ordering | `STAY` |
| `lifecycle/save_flow.rs` | 1,536 | System>Quit save-flow implementation | R20 |
| `lifecycle/task_tick.rs` | 410 | product recurring-task scheduling | `STAY` |
| `lifecycle/title_visual_startup.rs` | 142 | product startup arming/order | R22 |
...
| `mod.rs` | 110 | experiments module root and compatibility exports | `STAY` |
| `mod/own_stepper_idx6_memory.rs` | 9 | own-stepper memory family | D5 and R14 |
| `mod/product_core_own_stepper.rs` | 555 | product core own-stepper | D5 |
...
| `startup_hooks.rs` | 115 | product startup root and arming facade | `STAY` |
| `startup_hooks/diagnostics/layout_global_hooks.rs` | 357 | mixed title, quit, and product diagnostics | R11 and R22 |
| `startup_hooks/diagnostics/mod.rs` | 29 | diagnostics module facade | `STAY` |
...
| `startup_hooks/quit_menu/build_url_row.rs` | 81 | System>Quit "Load Build from URL" row: press -> `er-build-import-runtime::request`, FrameBegin tick -> `::tick` | R18 |
...
| `startup_hooks/quit_menu/mod.rs` | 40 | quit-menu module facade | R10-R20 |
| `startup_hooks/save_picker/profile_05_010_editor_runtime.rs` | 1,404 | R12B1-R12B5 families listed in section 4.2 | R12A-R12B5 |
| `startup_hooks/quit_menu/profile_rows_system_quit_menu.rs` | 1,189 | mixed profile-row title, quit, and sampler families | R11 |
| `lifecycle/save_dest_commit.rs` | 79 | product facade: implementation in `er_quit_menu_core::save_dest_commit_runtime`; this side supplies the save-redirect native source dir and the `er-save-suppress` save-job observer | R18 |
| `lifecycle/save_flow_boxes.rs` | 712 | System>Quit confirmation-box family | R18-R20 |
...
| `lifecycle/save_game_flow_handlers.rs` | 431 | split out of `system_quit_dialog_handlers.rs`: the vanilla Save Game row's request/retract/close flow, read by `lifecycle/save_flow.rs` | R18-R20 |
| `startup_hooks/diagnostics/scaleform_handler_hooks.rs` | 81 | split out of `system_quit_dialog_handlers.rs`: the handler ctor/dtor live-set, installed by `system_quit_ownership_repro.rs` beside it | R19 |
| `startup_hooks/save_picker/save_picker_path_editor.rs` | 10 | R13B1-R13B4 families listed in section 4.3 | R13A-R13B4 |
...
| `startup_hooks/diagnostics/system_quit_ownership_repro.rs` | 1,308 | ownership, telemetry, quit, and portrait families | R19 |
| `startup_hooks/quit_menu/system_quit_repro_guards.rs` | 730 | product repro guard and quit/title families | R2 and R19 |
...
| `startup_hooks/save_picker/save_picker_surface.rs` | 156 | picker surface routing adapter | R17-R18 |
| `title.rs` | 5 | title facade | R22 |
| `trace.rs` | 10 | trace facade | R6A-R6D and D4 |
| `trace/menu_constructor_capture.rs` | 1,402 | menu constructor capture family | R6B and D4 |
| `trace/menu_trace_hooks.rs` | 2,070 | title reload and menu trace families | R6C, R21, and D4 |
| `trace/native_result_map_hooks.rs` | 826 | native result-map hook family | R6A and D4 |
