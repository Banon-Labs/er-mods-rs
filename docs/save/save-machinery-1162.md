...
11. **Coexistence with `er_effects_rs.dll` and `er-reload-trace`.** Bare MinHook on `0x67b200`/`0x67b290` is forbidden. Do not write `CSMenuMan+0x13c`, `bc4`, `b72`, `b73`, `b78` or `b80` -- those are product-owned during a switch, and a second writer will fight it frame by frame. Strategy (c) satisfies all of this; strategies (a) and (b) do not.
