...
/// [`windows::Win32::Foundation::HWND`], or `None` if it is not a live window.
///
/// LOCAL PATCH (er-mods-rs, 2026-08-29). Upstream unwraps the `GetClientRect` result. Under
/// Wine/Proton with vkd3d-proton the HWND reached here -- `swap_chain.GetDesc().OutputWindow` of
/// whichever swap chain the Present hook saw first -- is sometimes not a live window, and
/// `GetClientRect` returns 0x80070578 ERROR_INVALID_WINDOW_HANDLE. The unwrap then panics inside
/// an `extern "system"` callback, which is an abort: it killed ELDEN RING twice on 2026-08-29,
/// once from er_build_watermark and once from er_net_effects, ~2s into boot. An overlay that
/// cannot size itself must decline to initialise, not take the process down.
pub fn try_win_size(hwnd: HWND) -> Option<(i32, i32)> {
...
/// Helper that returns width and height of a given
/// [`windows::Win32::Foundation::HWND`], zero-sized if the handle is not a live window.
pub fn win_size(hwnd: HWND) -> (i32, i32) {
    try_win_size(hwnd).unwrap_or((0, 0))
}
