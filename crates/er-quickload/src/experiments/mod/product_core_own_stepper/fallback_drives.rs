macro_rules! own_stepper_idx10_fallbacks {
    ...
    if $phase == OWN_STEPPER_PHASE_MENU_BUILD {
        ...
        // The `!own_stepper_passive_enabled() && !input_probe_enabled() && !live_dialog_enabled()`
        // terms that opened this condition were all permanently `false`, so every negation was
        // permanently `true`. All three gates are deleted.
