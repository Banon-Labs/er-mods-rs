...
//!
//! Output: `er-quickload-input-trace.jsonl` next to eldenring.exe, one JSON object per line:
...
//! `ms` matches the `[+Nms]` prefix clock of er-quickload-autoload-debug.log for cross-correlation.
...
pub(crate) fn input_trace_enabled() -> bool {
    matches!(
        std::env::var("ER_QUICKLOAD_INPUT_TRACE").as_deref(),
        Ok("1")
    ) || game_directory_path()
        .unwrap_or_else(|| PathBuf::from("."))
        .join("er-quickload-input-trace.txt")
        .exists()
}
...
fn input_trace_path() -> PathBuf {
    std::env::var_os("ER_QUICKLOAD_INPUT_TRACE_PATH")
        ...
        .unwrap_or_else(|| {
            game_directory_path()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("er-quickload-input-trace.jsonl")
        })
}
