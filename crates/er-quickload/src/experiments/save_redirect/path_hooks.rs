...
pub(crate) fn save_override_telemetry_only() -> bool {
    matches!(
        std::env::var("ER_QUICKLOAD_TELEMETRY_ONLY").as_deref(),
        Ok("1")
    ) || game_directory_path()
        .unwrap_or_else(|| PathBuf::from("."))
        .join("er-quickload-telemetry-only.txt")
        .exists()
}
...
pub(crate) fn save_trace_enabled() -> bool {
    matches!(std::env::var("ER_QUICKLOAD_SAVE_TRACE").as_deref(), Ok("1"))
        || game_directory_path()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("er-quickload-save-trace.txt")
            .exists()
}
...
const OWN_LOAD_UNRESOLVABLE_PROBE_ENV: &str = "ER_QUICKLOAD_PROBE_OWN_LOAD_UNRESOLVABLE";
...
fn configured_active_steam_id64_env() -> Option<u64> {
    [
        "ER_QUICKLOAD_ACTIVE_STEAMID",
        "ER_QUICKLOAD_ACTIVE_STEAM_ID64",
    ]
    .into_iter()
    .find_map(|name| {
        ...
        let is_steam_id =
            (16..=20).contains(&trimmed.len()) && trimmed.as_bytes().iter().all(u8::is_ascii_digit);
        is_steam_id
