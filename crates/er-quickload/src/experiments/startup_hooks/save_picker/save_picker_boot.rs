...
//
// WHAT THIS FIXES. `er-quickload.toml`'s `os_native_save_picker` governed the two System>Quit
...
pub(crate) fn boot_os_open_missing_save_picker() -> bool {
    ...
    let spawned = std::thread::Builder::new()
        .name("er-quickload-boot-os-picker".to_owned())
        .spawn(|| {
...
/// The dialog, and the one decision each outcome leads to. Runs on `er-quickload-boot-os-picker`.
...
pub(crate) fn boot_os_perform_cancel_exit() -> ! {
    ...
    append_autoload_debug(format_args!(
        ...
        if flushed {
            ...
            " -- the state mutex was not available, so er-quickload-telemetry.json predates this cancel; er-quickload-bootstrap.jsonl carries the outcome"
        }
    ));
