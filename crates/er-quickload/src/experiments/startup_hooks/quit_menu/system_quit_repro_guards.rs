...
/// `er-quickload-sq-target-slots.txt` = comma/space-separated slot indices (e.g. "0,2,3"). Switch i
...
pub(crate) fn sq_repro_target_slot() -> i32 {
    ...
    let path = game_directory_path()
        ...
        .join("er-quickload-sq-target-slots.txt");
...
pub(crate) fn sq_repro_target_switches() -> usize {
    ...
    // Uses a game-dir CONTROL FILE (`er-quickload-sq-target-switches.txt` = a decimal count), the same
    // sanctioned runtime-marker pattern as the other sq-repro modes -- NOT a new ER_QUICKLOAD_* env gate
    // (env gates are frozen by .auto/env_gate_comment_policy.rego). Absent/unparseable -> the const.
    let runtime = game_directory_path()
        .unwrap_or_else(|| PathBuf::from("."))
        .join("er-quickload-sq-target-switches.txt");
...
/// `er-quickload-system-quit-load-switch.txt` / `ER_QUICKLOAD_SQ_LOAD_SWITCH=1`. The ProfileSelect
...
// ENV-GATE RATIONALE: ER_QUICKLOAD_SQ_LOAD_SWITCH=1 selects the profile-load-switch repro autopilot (Quit
