...
//!
//! Its own module, outside the `#[cfg(windows)]` half
...
//! # The regression
//!
//! Until 2026-09-13 `stats_panel_enabled()` asked only `autoload_disabled()` and
//! `save_override_telemetry_only()`. That was sound while the boot autoload was the only consumer
//! that existed. It stopped being sound the moment `autoload` became a cargo feature that can be
//! left out: a build carrying the System>Quit rows and no boot autoload answers false at all
//! eleven `stats_panel_enabled()` call sites and serves the vanilla movie to a menu built for the
//! edited one.
//!
...
mod profile_select_chrome_gate_tests {
    ...
    /// The pairing this crate actually evaluates, not just the predicate: a build carrying the
    /// cloned rows wants the chrome whatever the boot autoload is doing.
    #[test]
    fn a_build_with_the_rows_wants_the_chrome_whatever_the_boot_autoload_does() {
        if !cfg!(feature = "quit-rows") {
            return;
        }
        for boot_autoload in [false, true] {
            assert!(
                profile_select_chrome_required(boot_autoload, true, false),
