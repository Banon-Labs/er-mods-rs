...
//! something is possessed -- a handful of struct-field writes on two `ChrIns`. It claims **no
//! game prologue** and patches no param, so it stays co-loadable with every other shell in this
//! profile. It resolves **exactly one game function address**:
//! `PlayAnimationByBehaviorName`, through `game_rva_named`, on the by-name animation fallback
//! only -- see [`possess::game`] for why every other lever turned out to be a struct field, and
//! `input` for why polling rather than hooking is the smaller claim rather than the lazier one.
//!
//! The one hook anywhere in the crate is hudhook's DX12 `Present`, which [`picker`] installs the
//! first time the creature list is opened and never otherwise -- a swapchain vtable slot rather
//! than a game prologue, arbitrated by `er_build_watermark_core::overlay_host` so the process
//! keeps exactly one of them however many shells want to draw.
//!
...
pub unsafe extern "system" fn DllMain(
    ...
    if reason == DLL_PROCESS_ATTACH {
        ...
        // GAME-IMAGE detour, so `er-hook` has no refusal to be silent about. (It does resolve one
        // game function address -- `PlayAnimationByBehaviorName` -- but that is a CALL through
        // `game_rva_named`, which logs its own refusal, not a detour.) The picker's overlay hooks
        // `Present` on the swapchain, which is not a game prologue and is arbitrated by
        // `er_build_watermark_core::overlay_host` rather than by `er-hook`. If a later layer
        // claims a game prologue, that call goes here.
