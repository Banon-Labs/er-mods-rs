//! Deterministic menu-input probe constants + the d180 leaf-tick counter.
//!
//! Moved verbatim from er-effects-rs `constants/loading_cover.rs` with the loading-cover crate
//! extraction and renamed to what it actually holds (the old filename predates the split of the
//! title-cover constants out of it). The root re-exports every name through its `constants.rs` glob.
...
pub use er_telemetry_core::counters::INPUT_PROBE_ACTIVE;
/// One-shot latch: set when the d180 leaf tick is observed during the HIGHLIGHT window (decisive).
pub use er_telemetry_core::counters::INPUT_PROBE_D180_PRECONFIRM;
...
pub use er_telemetry_core::counters::INPUT_PROBE_DOWN_LEAF_BASELINE;
...
pub use er_telemetry_core::counters::INPUT_PROBE_FRAME;
