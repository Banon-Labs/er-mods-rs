use crate::loading_cover_host::{
    create_absolute_hook, own_window, safe_input_proc, trace_first_game_caller_rva,
};
use crate::prelude::*;
...
pub use er_telemetry_core::counters::WINRECONFIG_CHANGE_DISPLAY_ORIG;
/// Trampolines (0 = hook not installed).
pub use er_telemetry_core::counters::WINRECONFIG_CREATE_WINDOW_ORIG;
pub use er_telemetry_core::counters::WINRECONFIG_MOVE_WINDOW_ORIG;
pub use er_telemetry_core::counters::WINRECONFIG_SET_WINDOW_LONG_ORIG;
pub use er_telemetry_core::counters::WINRECONFIG_SET_WINDOW_POS_ORIG;

pub use er_telemetry_core::counters::WINRECONFIG_CHANGE_DISPLAY_CALLS;
/// Total call counts (telemetry: the reconfig timeline's RAM counters).
pub use er_telemetry_core::counters::WINRECONFIG_CREATE_WINDOW_CALLS;
/// Last SetWindowPos geometry, packed (cx << 32 | cy) and (x << 32 | y as u32) for telemetry.
pub use er_telemetry_core::counters::WINRECONFIG_LAST_SET_POS_SIZE;
pub use er_telemetry_core::counters::WINRECONFIG_MOVE_WINDOW_CALLS;
pub use er_telemetry_core::counters::WINRECONFIG_SET_WINDOW_LONG_CALLS;
pub use er_telemetry_core::counters::WINRECONFIG_SET_WINDOW_POS_CALLS;

/// Per-hook log cap: the first calls carry the whole startup story; later calls only count.
pub const WINRECONFIG_LOG_CAP: usize = 48;
/// Class/name pointers below this are ATOM values, not strings (Win32 MAKEINTATOM contract).
pub const WINRECONFIG_ATOM_LIMIT: usize = 0x1_0000;
/// Bounded UTF-16 read for window/class names.
pub const WINRECONFIG_NAME_CAP: usize = 64;
/// DEVMODEW fixed ABI offsets (dmPelsWidth / dmPelsHeight); read raw so no Gdi feature is needed.
pub const DEVMODEW_PELS_WIDTH_OFFSET: usize = 0xAC;
pub const DEVMODEW_PELS_HEIGHT_OFFSET: usize = 0xB0;

pub fn winreconfig_name(ptr: usize) -> String {
    if ptr == 0 {
...
pub type CreateWindowExWFn = unsafe extern "system" fn(
    u32,
    usize,
    usize,
    u32,
...
/// # Safety
///
/// A MinHook detour: the OS calls it with the real user32 arguments, and it forwards them to the
/// trampoline stored in the matching `*_ORIG` slot. It must only be installed over the user32
/// export whose ABI it mirrors, and that slot must hold that export's trampoline.
pub unsafe extern "system" fn winreconfig_create_window_hook(
    exstyle: u32,
    class: usize,
    name: usize,
    style: u32,
...
pub type SetWindowPosFn = unsafe extern "system" fn(usize, usize, i32, i32, i32, i32, u32) -> i32;
...
/// trampoline stored in the matching `*_ORIG` slot. It must only be installed over the user32
/// export whose ABI it mirrors, and that slot must hold that export's trampoline.
pub unsafe extern "system" fn winreconfig_set_window_pos_hook(
    hwnd: usize,
    insert_after: usize,
    x: i32,
    y: i32,
...
pub type SetWindowLongPtrWFn = unsafe extern "system" fn(usize, i32, isize) -> isize;
...
/// trampoline stored in the matching `*_ORIG` slot. It must only be installed over the user32
/// export whose ABI it mirrors, and that slot must hold that export's trampoline.
pub unsafe extern "system" fn winreconfig_set_window_long_hook(
    hwnd: usize,
    index: i32,
    value: isize,
) -> isize {
...
pub type MoveWindowFn = unsafe extern "system" fn(usize, i32, i32, i32, i32, i32) -> i32;
...
/// trampoline stored in the matching `*_ORIG` slot. It must only be installed over the user32
/// export whose ABI it mirrors, and that slot must hold that export's trampoline.
pub unsafe extern "system" fn winreconfig_move_window_hook(
    hwnd: usize,
    x: i32,
    y: i32,
    w: i32,
...
pub type ChangeDisplaySettingsExWFn =
...
/// trampoline stored in the matching `*_ORIG` slot. It must only be installed over the user32
/// export whose ABI it mirrors, and that slot must hold that export's trampoline.
pub unsafe extern "system" fn winreconfig_change_display_hook(
    devname: usize,
    devmode: usize,
    hwnd: usize,
    flags: u32,
...
pub fn install_window_reconfig_observer_hooks() {
...
pub use er_telemetry_core::counters::WINRECONFIG_EARLY_APPLY_MS;
pub use er_telemetry_core::counters::WINRECONFIG_EARLY_APPLY_RECT;
...
pub use er_telemetry_core::counters::WINRECONFIG_EARLY_APPLY_RESULT;

pub const WINRECONFIG_EARLY_APPLY_MAX_MS: u128 = 20_000;
pub const WINRECONFIG_EARLY_APPLY_POLL_MS: u64 = 20;
pub const WINRECONFIG_RESULT_APPLIED: usize = 1;
pub const WINRECONFIG_RESULT_SKIP_WINDOWED: usize = 2;
pub const WINRECONFIG_RESULT_NO_WINDOW: usize = 3;
pub const WINRECONFIG_RESULT_NO_MONITOR: usize = 4;
pub const WINRECONFIG_RESULT_NO_CONFIG: usize = 5;
pub const WINRECONFIG_RESULT_ALREADY_FINAL: usize = 6;
pub const CSIDL_APPDATA: i32 = 0x1a;
pub const SHGFP_TYPE_CURRENT: u32 = 0;
pub const MAX_PATH_W: usize = 260;

pub type WinreconfigShGetFolderPathWFn =
...
pub fn winreconfig_screen_mode() -> Option<String> {
...
pub fn winreconfig_finish(result: usize, since_ms: u128, detail: &str) {
...
pub fn apply_startup_window_final_geometry() {
