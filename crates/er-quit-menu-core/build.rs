//! The message-box and save-request prologues this crate byte-checks, from named instructions.
//!
//! They were generated by `er-quickload`'s build script until 2026-09-12, which put the Save Game
//! row's recipe resolver out of reach of every other host: the constants were `pub(crate)` to a
//! crate this one cannot depend on. Generating them here, `pub`, is what lets the row's flow live
//! beside the chrome it drives -- and `er-quickload` now imports them back from this crate, so
//! there is still exactly one generator and one set of bytes.
//!
