...
use er_telemetry_core::counters::{
    SAVE_DEST_COMMIT_FAIL, SAVE_DEST_COMMIT_PENDING, SAVE_DEST_DISARM_DEFERRED,
    SAVE_DEST_FOREIGN_OPEN_PASSED, SAVE_DEST_LIVE_BAK_MUTATED, SAVE_DEST_LIVE_FILE_MUTATED,
    SAVE_DEST_LIVE_OVERWRITE_COUNT, SAVE_DEST_LIVE_STAT_UNREADABLE, SAVE_DEST_OPEN_PICKER_PENDING,
    SAVE_DEST_REDIRECT_ARMED, SAVE_DEST_REDIRECT_HITS, SAVE_DEST_RESTORE_FAILED,
    SAVE_DEST_RESTORE_SUPPRESSED, SAVE_DEST_SEED_FAIL_COUNT, SAVE_DEST_SEEDED_COUNT,
    SAVE_DEST_TARGET_STRUCTURE_OK, SAVE_DEST_TARGET_WRITTEN_OK,
};

use crate::host::{append_autoload_debug, system_quit_env_save_path};
use crate::row_text::system_quit_path_for_windows;
use crate::save_dest_commit::{
    SAVE_DEST_BND4_MAGIC, save_dest_accepted_dirs_for, save_dest_accepted_leaves,
    save_dest_accepted_paths_for, save_dest_bak_path, save_dest_container_end,
    save_dest_is_write_access, save_dest_wide_leaf_lower,
};
use crate::save_dest_identity::{
    SaveDestIdentity, save_dest_dir_identity, save_dest_file_identity, save_dest_normalize_wide,
    save_dest_normalized_parent, save_dest_write_atomic,
};

/// The SL save-job body observer, injected at the call site.
///
/// The counters live in `er-save-suppress`, which owns save suppression and stays OUTSIDE this
/// crate (see the boundary note in `lib.rs`). They are read at the exact points the moved code
/// always read them -- these are the same three functions, reached through a pointer instead of a
/// path -- so a log line still quotes the value the writer had when the line was built.
#[derive(Clone, Copy)]
pub struct SaveJobObserver {
    /// True when no save-job body is executing right now.
    pub writer_idle: fn() -> bool,
    /// Save-job bodies entered since process start.
    pub starts: fn() -> u64,
    /// Save-job bodies that have RETURNED since process start.
    pub completions: fn() -> u64,
}
...
pub static SAVE_DEST_TARGET_PATH: Mutex<Option<PathBuf>> = Mutex::new(None);

/// Live save + destination bookkeeping for one in-flight commit.
pub struct SaveDestRedirect {
    target_path: PathBuf,
...
pub static SAVE_DEST_REDIRECT: Mutex<Option<SaveDestRedirect>> = Mutex::new(None);
...
pub struct SaveDestLiveOverwrite {
    live_path: PathBuf,
...
pub static SAVE_DEST_LIVE_OVERWRITE: Mutex<Option<SaveDestLiveOverwrite>> = Mutex::new(None);
...
pub struct SaveDestVerdict {
    pub ok: bool,
    pub summary: String,
}

pub fn save_dest_target_lock() -> std::sync::MutexGuard<'static, Option<PathBuf>> {
    SAVE_DEST_TARGET_PATH
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
}

pub fn save_dest_redirect_lock() -> std::sync::MutexGuard<'static, Option<SaveDestRedirect>> {
    SAVE_DEST_REDIRECT
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
}

pub fn save_dest_live_overwrite_lock()
...
pub fn save_dest_set_target(path: PathBuf, source: &str) {
...
pub fn save_dest_reset(reason: &str, observer: SaveJobObserver) {
    ...
    if SAVE_DEST_REDIRECT_ARMED.load(Ordering::SeqCst) != 0 {
        ...
        let _ = save_dest_verify_and_disarm(reason, observer);
...
pub fn save_dest_file_stamp(path: &Path) -> Option<(u64, u128)> {
...
pub fn save_dest_arm_live_overwrite(live_path: &Path, reason: &'static str) {
...
pub fn save_dest_arm_redirect(
    ...
    let accepted_paths = save_dest_accepted_paths_for(live_path, native_source_dir.clone());
    ...
    let accepted_dirs = save_dest_accepted_dirs_for(live_path, native_source_dir);
...
pub fn save_dest_redirect_armed() -> bool {
...
pub fn save_dest_commit_window_armed() -> bool {
...
pub static SAVE_DEST_DEFER_REPORTED: AtomicUsize = AtomicUsize::new(0);

/// Where the native SL writer is, relative to the commit that armed this window.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum SaveDestWriterState {
...
pub fn save_dest_writer_state(
    completions_at_fire: u64,
    observer: SaveJobObserver,
) -> SaveDestWriterState {
    if !(observer.writer_idle)() {
        return SaveDestWriterState::InBody;
    }
    if (observer.completions)() > completions_at_fire {
...
pub fn save_dest_reset_defer_report() {
...
pub fn save_dest_teardown_allowed(
    completions_at_fire: u64,
    context: &str,
    observer: SaveJobObserver,
) -> bool {
    ...
    if save_dest_writer_state(completions_at_fire, observer) != SaveDestWriterState::InBody {
        return true;
    }
...
pub fn save_dest_redirect_for_open(path: &[u16], access: u32) -> Option<Vec<u16>> {
...
/// catches that.
pub fn save_dest_note_redirect_hit(handle_ok: bool) {
...
pub fn save_dest_verify_and_disarm(
    reason: &str,
    observer: SaveJobObserver,
) -> Option<SaveDestVerdict> {
    if save_dest_commit_window_armed() && !(observer.writer_idle)() {
        SAVE_DEST_DISARM_DEFERRED.fetch_add(1, Ordering::SeqCst);
        append_autoload_debug(format_args!(
...
pub fn save_dest_verify_destination(state: &SaveDestRedirect, reason: &str) -> SaveDestVerdict {
...
pub fn save_dest_score_live_file(state: &SaveDestRedirect, reason: &str) -> SaveDestLiveState {
...
pub fn save_dest_verify_live_overwrite(
...
pub fn save_dest_live_save_path() -> Option<PathBuf> {
...
pub fn save_dest_commit_identity(target: &Path, live: &Path) -> SaveDestIdentity {
