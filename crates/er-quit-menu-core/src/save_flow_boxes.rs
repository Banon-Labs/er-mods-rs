...
use crate::prologues::MENU_JOB_EMIT_RESULT_SIG;
use crate::prologues::MENU_JOB_EMIT_RESULT_SIG_MASK;
use crate::prologues::SYSTEM_QUIT_MSGBOX_ADD_NO_SIG;
use crate::prologues::SYSTEM_QUIT_MSGBOX_ADD_NO_SIG_MASK;
use crate::prologues::SYSTEM_QUIT_MSGBOX_ADD_YES_SIG;
use crate::prologues::SYSTEM_QUIT_MSGBOX_ADD_YES_SIG_MASK;
use crate::prologues::SYSTEM_QUIT_MSGBOX_BUILDER_CTOR_SIG;
use crate::prologues::SYSTEM_QUIT_MSGBOX_BUILDER_CTOR_SIG_MASK;
use crate::prologues::SYSTEM_QUIT_MSGBOX_DEFAULT_LAST_SIG;
use crate::prologues::SYSTEM_QUIT_MSGBOX_DEFAULT_LAST_SIG_MASK;
use crate::prologues::SYSTEM_QUIT_MSGBOX_DTOR_SIG;
use crate::prologues::SYSTEM_QUIT_MSGBOX_DTOR_SIG_MASK;
use crate::prologues::SYSTEM_QUIT_MSGBOX_FINALIZE_SIG;
use crate::prologues::SYSTEM_QUIT_MSGBOX_FINALIZE_SIG_MASK;
use crate::software_keyboard::mh_install_hook_once;
...
use er_telemetry_core::counters::MENU_JOB_EMIT_RESULT_INSTALLED;
use er_telemetry_core::counters::SAVE_FLOW_BOX_DIALOG;
use er_telemetry_core::counters::SAVE_FLOW_BOX_EMIT_COUNT;
use er_telemetry_core::counters::SAVE_FLOW_BOX_EMIT_DIALOG;
use er_telemetry_core::counters::SAVE_FLOW_BOX_EMIT_STATE;
use er_telemetry_core::counters::SAVE_FLOW_BOX_EXPECTED;
use er_telemetry_core::counters::SAVE_FLOW_BOX_HOST_DIALOG;
use er_telemetry_core::counters::SAVE_FLOW_BOX_IDENTITY_LOST_COUNT;
use er_telemetry_core::counters::SAVE_FLOW_BOX_NO_COUNTS;
use er_telemetry_core::counters::SAVE_FLOW_BOX_OPEN_COUNTS;
use er_telemetry_core::counters::SAVE_FLOW_BOX_RESULT_BASELINE;
use er_telemetry_core::counters::SAVE_FLOW_BOX_UNDECIDABLE_COUNTS;
use er_telemetry_core::counters::SAVE_FLOW_BOX_YES_COUNTS;
use er_telemetry_core::counters::SAVE_FLOW_DIALOG;
use er_telemetry_core::counters::SAVE_FLOW_RECIPE_UNAVAILABLE;
use er_telemetry_core::counters::SAVE_FLOW_SUBMIT_BOX_PENDING;
use er_title_flow::HOOK_ORIGINAL_UNSET;
use er_title_flow::MENU_JOB_EMIT_RESULT_INSTALLED_YES;
use er_title_flow::MENU_JOB_EMIT_RESULT_NOT_INSTALLED;
use er_title_flow::MENU_JOB_EMIT_RESULT_ORIG;
use er_title_flow::MENU_JOB_EMIT_RESULT_RVA;
use er_title_flow::MENU_JOB_QUEUE_READY_RVA;
use er_title_flow::MENU_JOB_RESULT_STATE_CONTINUE_MAX;
use er_title_flow::MENU_JOB_RESULT_STATE_NONE;
use er_title_flow::MENU_JOB_SUBMIT_RVA;
use er_title_flow::MENU_STRING_FROM_WIDE_RVA;
use er_title_flow::MENU_STRING_SIZE;
use er_title_flow::MSGBOX_BUILDER_BUTTON_COUNT_OFF;
use er_title_flow::MSGBOX_BUILDER_CTOR_TRAILING_ARG;
use er_title_flow::MSGBOX_BUILDER_DEFAULT_IDX_OFF;
use er_title_flow::MSGBOX_BUILDER_MODE_CONFIRM;
use er_title_flow::MSGBOX_BUILDER_ORIG;
use er_title_flow::MSGBOX_BUILDER_SIZE;
use er_title_flow::MSGBOX_BUTTON_COUNT_25E8_OFFSET;
use er_title_flow::MSGBOX_CLOSING_LATCH_3B0_OFFSET;
use er_title_flow::MSGBOX_DEFAULT_CURSOR_25E0_OFFSET;
use er_title_flow::MSGBOX_DIALOG_UPDATE_RVA;
use er_title_flow::MSGBOX_DIALOG_VTABLE_UPDATE_SLOT;
use er_title_flow::MSGBOX_JOB_RESULT_STATE_1E8_OFFSET;
use er_title_flow::MSGBOX_JOB_RESULT_SUBCODE_1EC_OFFSET;
use er_title_flow::SYSTEM_QUIT_DIALOG_MENU_JOB_QUEUE_10_OFFSET;
use er_title_flow::SYSTEM_QUIT_DIALOG_MENU_WINDOW_LIST_50_OFFSET;
use er_title_flow::SYSTEM_QUIT_MSGBOX_ADD_NO_RVA;
use er_title_flow::SYSTEM_QUIT_MSGBOX_ADD_YES_RVA;
use er_title_flow::SYSTEM_QUIT_MSGBOX_BUILDER_CTOR_RVA;
use er_title_flow::SYSTEM_QUIT_MSGBOX_DEFAULT_LAST_RVA;
use er_title_flow::SYSTEM_QUIT_MSGBOX_DTOR_RVA;
use er_title_flow::SYSTEM_QUIT_MSGBOX_FINALIZE_RVA;
use er_title_flow::TITLE_OWNER_SCAN_START_ADDRESS;
use er_title_flow::{MENU_JOB_RESULT_STATE_FAILED, MENU_JOB_RESULT_STATE_SUCCESS};
use std::ffi::c_void;
...
// ===== moved from `er-quickload`'s `experiments::lifecycle` on 2026-09-12 =====
// The Save Game row's flow, so a host that is not the product can carry the row. The chrome
// it drives and the router that presses it were already here; only the flow was still behind
// the product's own module tree, which is what made `er_save_game_row.dll` impossible.
...
// The re-export that used to stand here is gone: the ids, button order and answer enums it pulled
// out of this crate are now defined a few hundred lines above, because the native
// MessageBoxBuilder integration joined them here on 2026-09-12 instead of staying in the product.
...
#[repr(C, align(16))]
pub struct SaveFlowMsgBoxBuilderScratch {
    bytes: [u8; MSGBOX_BUILDER_SIZE],
}

/// Stack scratch for the prompt `CS::MenuString`.
#[repr(C, align(8))]
pub struct SaveFlowMenuStringScratch {
    bytes: [u8; MENU_STRING_SIZE],
}

pub static SAVE_FLOW_BOX_RECIPE: OnceLock<Option<SaveFlowBoxRecipe>> = OnceLock::new();
...
pub fn save_flow_verify_rva(rva: u32, expected: &[u8], mask: &[u8], name: &str) -> Option<usize> {
...
pub fn save_flow_verify_rva_for_hook(
    rva: u32,
    expected: &[u8],
    mask: &[u8],
    name: &str,
...
pub fn save_flow_box_recipe() -> Option<&'static SaveFlowBoxRecipe> {
...
pub fn save_flow_box_recipe_available() -> bool {
...
pub fn save_flow_box_host_dialog() -> usize {
...
pub fn save_flow_box_set_host_dialog(dialog: usize) {
...
/// function.
pub fn save_flow_box_identity(dialog: usize, base: usize) -> (usize, usize, bool) {
...
pub struct SaveFlowBoxSnapshot {
    vtable: usize,
    update_slot: usize,
    result_state: i32,
    result_subcode: i32,
...
pub fn save_flow_box_result_baseline() -> i32 {
...
pub fn save_flow_box_finish(box_id: usize, decision: SaveFlowDecision) -> SaveFlowDecision {
    SAVE_FLOW_BOX_DIALOG.store(0, Ordering::SeqCst);
...
pub fn save_flow_box_emitted_state(dialog: usize) -> i32 {
...
pub fn save_flow_box_poll_fingerprint(dialog: usize, snapshot: &SaveFlowBoxSnapshot) -> usize {
    let mut hash = dialog;
    for value in [
...
pub fn save_flow_box_trace_poll(box_id: usize, dialog: usize, snapshot: &SaveFlowBoxSnapshot) {
...
pub static SAVE_FLOW_BOX_LAST_POLL: AtomicUsize = AtomicUsize::new(0);
...
pub static SAVE_FLOW_BOX_SUBMIT_DEFERRED: AtomicUsize = AtomicUsize::new(SAVE_FLOW_BOX_NONE);
...
pub fn install_menu_job_emit_result_hook() {
...
pub fn save_flow_box_note_build(box_id: usize, dialog: usize) {
    let baseline =
...
/// The save-flow half of the `CS::MessageBoxDialog` builder detour, owned by this crate.
///
/// The whole detour lives in `er-quickload`, where it also drives the startup auto-accept and the
/// product's msgbox suppression. Neither of those belongs to a shell, and the save flow needs only
/// the first branch of it: when [`save_flow_submit_box`] has tagged a box id, forward the build and
/// record the dialog it produced. Everything that branch reads is in this module already.
///
/// Without it a shell reports `builder_capture_live=false` and the overwrite confirm cannot be
/// built, so picking an existing destination file is refused rather than confirmed -- measured on
/// runs br-20260912-194412-7743 through br-20260912-201935-ad27.
///
/// Forwarding untagged builds unchanged is the point: this handler must never suppress a dialog,
/// because the one it would eat is the player's own.
///
/// # Safety
///
/// Installed by `er-hook`; the game calls it on its menu thread with the builder's four arguments.
unsafe extern "system" fn save_flow_msgbox_builder_hook(
    a: usize,
    b: usize,
    c: usize,
    d: usize,
    ...
    let next: unsafe extern "system" fn(usize, usize, usize, usize) -> usize =
        unsafe { std::mem::transmute(orig) };
    let ret = unsafe { next(a, b, c, d) };
    let expected_box = SAVE_FLOW_BOX_EXPECTED.load(Ordering::SeqCst);
    if expected_box == SAVE_FLOW_BOX_NONE || ret == 0 {
        return ret;
    }
    let Ok(base) = er_game_base::mem::game_module_base() else {
        return ret;
    };
    // Structural identity, not a single vtable equality: the box must be recognised by every
    // vtable it can legitimately carry, which is the same check the decision poll uses.
    let (vt, update_slot, identity_ok) = save_flow_box_identity(ret, base);
    if identity_ok {
...
/// Claim the builder for the save-flow confirm, for a host that does not install its own detour.
///
/// Idempotent and chained through the union, so a host that already owns the address (the product
/// installs the full detour from its own copy) keeps its handler and this one runs beside it.
pub fn install_save_flow_msgbox_builder_capture() {
    if SAVE_FLOW_BUILDER_CLAIMED.swap(1, Ordering::SeqCst) != 0 {
        return;
    }
    let Ok(addr) = er_game_base::mem::game_rva_for_hook(er_title_flow::MSGBOX_BUILDER_RVA) else {
        append_autoload_debug(format_args!(
            "save-flow-box: failed to resolve the MessageBoxDialog builder rva 0x{:x}; the overwrite confirm cannot be built and an existing destination file will be refused",
            er_title_flow::MSGBOX_BUILDER_RVA
        ));
        SAVE_FLOW_BUILDER_CLAIMED.store(0, Ordering::SeqCst);
        return;
    };
    // Safety: the builder's ABI is the union's four-register shape, and `MSGBOX_BUILDER_ORIG` is
    // the static the handler above reads to call through.
    match unsafe {
        er_hook::register_union_hook(addr, save_flow_msgbox_builder_hook, &MSGBOX_BUILDER_ORIG)
    } {
        Ok(()) => append_autoload_debug(format_args!(
            "save-flow-box: registered the MessageBoxDialog builder 0x{addr:x} on the union; the overwrite confirm can be built and captured"
        )),
        Err(status) => {
            append_autoload_debug(format_args!(
                "save-flow-box: register_union_hook MessageBoxDialog builder failed: {status:?}; an existing destination file will be refused rather than confirmed"
            ));
            SAVE_FLOW_BUILDER_CLAIMED.store(0, Ordering::SeqCst);
        }
    }
...
/// One owner of the capture per process.
static SAVE_FLOW_BUILDER_CLAIMED: core::sync::atomic::AtomicUsize =
