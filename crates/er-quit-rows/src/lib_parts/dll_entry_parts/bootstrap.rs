...
pub unsafe extern "C" fn DllMain(hmodule: HINSTANCE, reason: u32, _reserved: *mut c_void) -> i32 {
    ...
    er_loading_portrait_core::install_host(er_loading_portrait_core::PortraitHost {
        ...
        configured_autoload_slot: crate::config::configured_autoload_slot,
        ensure_profile_slot_stats_cached: crate::experiments::ensure_profile_slot_stats_cached,
        profile_slot_name: crate::experiments::profile_slot_name,
        profile_slot_level: crate::experiments::profile_slot_level,
        ...
        // The cover's own frame. Without `loading-cover` the seam keeps its neutral default and
        // the portrait host renders nothing of ours over the game's loading screen.
        #[cfg(feature = "loading-cover")]
        boot_view_render_frame: crate::experiments::boot_view_render_frame,
        // Every remaining field keeps the crate's neutral default. Only reachable without
        // `loading-cover`, where that is the whole point: no frame source, nothing drawn.
    ...
    er_quit_menu_core::install_host(er_quit_menu_core::QuitMenuHost {
        ...
        // Only a build carrying the rows has an import to be told about; without them the seam
        // keeps its neutral default from `defaults()` below.
    ...
    // Nothing to provide when the cover is not compiled in: the compositor keeps no frame source
    // and every present passes the game's own backbuffer through untouched.
    #[cfg(feature = "loading-cover")]
    ...
    START_SAVE_FILE_OPS_CORE.call_once(|| {
        let _ = std::thread::Builder::new()
            .name("er-quickload-save-file-core".to_owned())
        ...
        SaveOverrideMode::TelemetryOnly => {
            if save_trace_enabled() {
                START_SAVE_REDIRECT.call_once(|| {
                    let _ = std::thread::Builder::new()
                        .name("er-quickload-save-trace".to_owned())
    ...
    if missing_save_gate_pending {
        ...
        std::thread::Builder::new()
            .name("er-quickload-missing-save-progress-gate".to_owned())
    ...
    if splash_skip_enabled() {
        START_SPLASH_SKIP.call_once(|| {
            let _ = std::thread::Builder::new()
                .name("er-quickload-splash-skip".to_owned())
    ...
    if online_disable_enabled() {
        START_ONLINE_DISABLE.call_once(|| {
            let _ = std::thread::Builder::new()
                .name("er-quickload-online-disable".to_owned())
...
fn spawn_save_suppress_install() {
    let _ = std::thread::Builder::new()
        .name("er-quickload-save-suppress".to_owned())
        .spawn(|| {
            ...
            // The product opt-in is the er-quickload.toml guard above, not an env var or a
