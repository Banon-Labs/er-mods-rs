...
// This crate is a copy of `er-quickload` being reduced to the System>Quit rows, and the two must
// never be told apart by guesswork. It writes its own file: a run directory holding both names
// says which DLL produced which line, and a run holding one says which DLL was loaded.
const AUTOLOAD_DEBUG_LOG_FILE_NAME: &str = "er-quit-rows-debug.log";
