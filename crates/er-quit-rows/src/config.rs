...
///
/// The cover is the only reader: the image is what it draws behind the bar and the portrait. The
/// `er-quickload.toml` key stays accepted and simply goes unread in a build without it.
...
/// Folder the missing-save picker opens in, from `er-quickload.toml` only (no env form on purpose:
...
/// `er-quickload.toml`. Defaults to true when the key is absent.
    ...
    *SESSION_PREFERRED_PICKER_DIR
    ...
    let Some(config) = runtime_config() else {
        append_autoload_debug(format_args!(
            "runtime-config: not persisting {PREFERRED_PICKER_DIR_KEY} -- config was unreadable/invalid at attach; fix er-quickload.toml first"
        ));
        return;
    };
...
///
/// The name is derived from the DLL (`er_quickload.dll` -> `er_quickload.toml`) rather than
/// being `er-quickload.toml`, so it cannot collide with the legacy DLL-adjacent config above.
/// That matters: stale `er-quickload.toml` files are strewn through `target/` and worktree build
...
mod tests {
    ...
    fn parse(contents: &str) -> Result<RuntimeConfig, String> {
        parse_runtime_config(PathBuf::from("C:\\Game\\er-quickload.toml"), contents)
    }

    #[test]
    fn linux_absolute_picker_directory_maps_to_wine_z_drive() {
        let config =
            parse("preferred_save_picker_dir = '/home/banon/projects/er-mods-rs/save-files'\n")
        ...
        assert_eq!(
            config
                ...
                .to_string_lossy(),
            "Z:\\home\\banon\\projects\\er-mods-rs\\save-files"
        );
    }
