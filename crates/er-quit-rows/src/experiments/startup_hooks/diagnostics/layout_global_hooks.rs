...
pub(crate) fn install_system_quit_duplicate_button_hook() {
    ...
    // a subset and supplies none, so a row whose flow it lacks is never on the tab.
    //
    // The set follows the `build-rows` feature rather than a runtime check, because the two are one
    // statement: a build without that feature has no link field, no importer call and no export, so
    // the two rows it would put on the tab could only ever log a press. A row that is not in the set
    // is never cloned.
    let armed = unsafe {
        er_quit_menu_core::row_cloner::arm(
            er_quit_menu_core::row_cloner::RowSet {
                load_build_from_url: cfg!(feature = "build-rows"),
                generate_build_link: cfg!(feature = "build-rows"),
