...
// Its call site in the file-open observer is commented out pending the delete pass.
#[allow(dead_code)]
    ...
    unsafe {
        text_input_02_990_swap(
            ...
            &TextInput02990Derivation {
                ...
                derive: |vanilla| {
                    let derived = er_gfx::build_url_02_990::centered_build_url_editor(vanilla)
                        .map_err(|error| error.to_string());
                    // Read the dim back out of the payload this is about to install, so a
                    // derivation that lost it is a counter rather than an undimmed field. The
                    // attestation lives with the backdrop, so a build without the build rows has
                    // no dim to attest.
                    #[cfg(feature = "build-rows")]
                    let derived = derived.and_then(crate::attest_derived_build_url_backdrop);
                    derived
                },
            },
        )
...
pub(crate) unsafe extern "system" fn title_scaleform_file_open_observer_hook(
    ...
    let ret = if base != null {
        if orig != null && orig != HOOK_ORIGINAL_UNSET {
            ...
            // Three of the five movie edits this observer used to serve are commented out rather
            // than deleted, pending the runtime test that proves the remaining two are enough.
            // This crate is being reduced to the System>Quit rows -- Save Game, Load Character and
            // Load Character from File, plus the submenus they open -- and a movie edit that
            // belongs to none of them is a change to the player's UI that nothing here asks for.
            // Their derive functions keep `#[allow(dead_code)]` so the code stays readable and
            // revivable until the delete pass.
            //
            // `02_990_TextInput_BuildUrl`, the link field. Its row left with `build-rows`, so
            //    the game can no longer ask for this cache key at all.
            // if is_build_url_02_990 {
            //     memory_replacement = unsafe { text_input_02_990_swap_to_build_url(base, native) };
            // }

            // `05_010_ProfileSelect`: the stats-panel edit -- face box removed, `ErStats` field
            // added, left column reflowed. ProfileSelect is the submenu Load Character opens, so
            // its contents are in scope and stay ours. Commenting this out was a regression: it
            // reverted that submenu to vanilla, which is not what trimming out-of-scope GFx means.
            ...
            // System->Quit six-cell grid: the three rows are cells of it, so without this edit
            // there are no rows to press. The one edit this crate exists for.
            if is_options_02_040 {
                memory_replacement = unsafe { options_02_040_quit6_swap_to_edited(base, native) };
            }
            // The save browser's CurrentPath field, which is the submenu Load Character from File
            // opens. In scope by the same rule that keeps the grid.
