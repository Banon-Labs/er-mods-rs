...
# The tmp-script guard has NO import of .cupcake/system, so it runs standalone -- and it was the
# one policy whose unit tests check.sh never invoked, which is precisely how its Bash branch
# stayed broken (first inert, then over-firing on reads) without a red gate.
command -v opa >/dev/null 2>&1 && opa test "$repo_root/.cupcake/policies/claude/edit_no_tmp_scripts_guard.rego" "$repo_root/.cupcake/tests/edit_no_tmp_scripts_guard_test.rego"
