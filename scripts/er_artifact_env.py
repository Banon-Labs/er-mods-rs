...
ARTIFACT_ENV: dict[str, str] = {
    ...
    # er-quit-menu's log. It had no knob until 2026-09-12, and it cost a diagnosis: on run
    # br-20260912-183117-e19a `er_quit_menu.dll` faulted inside the System-window restore, and the
    # line naming the window it died on -- `window=0x1dc3d080 vt=0x0 invalid` -- was in the
    # game-directory copy while the run's own directory held twenty-five files that said nothing
    # about it. The crash log's module backtrace named the shell; only the shell's own log named
    # the pointer. `er-quit-load-character` and `er-save-game-row` were two sibling standalone
    # shells that later merged into this one, which now reads its row set from a config file; their
    # own artifact-log knobs went with them.
