...
EXPECTED_NATIVE_CONTINUE = """# Copy to er-quickload-native-continue.txt next to eldenring.exe to enable the supported
# zero-input native Continue path.
"""

EXPECTED_PAB_ADVANCE = """# Copy to er-quickload-pab-advance.txt next to eldenring.exe to enable the supported
# zero-input press-any-button/menu-open advance.
"""

EXPECTED_SPLASH_SKIP = """# Copy this file to er-quickload-splash-skip.txt next to eldenring.exe to enable
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="er-quickload-autoload-stage-") as tmp:
        tmp_path = Path(tmp)
        er_dll = tmp_path / "er_quickload.dll"
        er_dll.write_bytes(b"fake er effects dll\n")
        out = tmp_path / "release"

        env = os.environ.copy()
        env["ER_QUICKLOAD_DLL"] = str(er_dll)
        ...
        if (out / "er-quickload.me3").read_text(encoding="utf-8") != EXPECTED_PROFILE:
            raise SystemExit("er-quickload.me3 is not the clean single-native profile (relative DLL path)")
        if (out / "er-quickload-autoload.txt.example").read_text(encoding="utf-8") != EXPECTED_AUTOLOAD:
            raise SystemExit("autoload example must keep direct_menu_load/product_core off by default")
        if (out / "er-quickload-native-continue.txt.example").read_text(encoding="utf-8") != EXPECTED_NATIVE_CONTINUE:
            raise SystemExit("native-continue example does not document the supported zero-input path")
        if (out / "er-quickload-pab-advance.txt.example").read_text(encoding="utf-8") != EXPECTED_PAB_ADVANCE:
            raise SystemExit("pab-advance example does not document the supported zero-input path")
        if (out / "er-quickload-splash-skip.txt.example").read_text(encoding="utf-8") != EXPECTED_SPLASH_SKIP:
