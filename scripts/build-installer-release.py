...
That variable must be an absolute path. The installer crate's build script runs with cargo's own
working directory, which is the crate directory and not the workspace root, so a relative path
resolves under the crate and finds nothing there. The build then fails with a list of DLL names
under "Build them first" -- which reads as a missing payload and is really a mislaid one, because
the payload it is looking past is sitting in the workspace `target/`. Measured 2026-09-23: the
relative spelling this block used to print failed both host builds while every DLL it named was on
disk, and the packaging step then shipped the previous run's binaries without complaint.
"""
...
def stage(out_dir: Path, source: Path, host_source: Path, commit: str) -> tuple[Path, list[str]]:
    ...
    if missing:
        raise SystemExit(
            ...
            "installer crate's own directory, so a relative path resolves under it, not here):\n"
            "  scripts/er-build-dlls.sh --all\n"
            "  ER_INSTALLER_EMBED_DIR=$PWD/target/x86_64-pc-windows-msvc/release \\\n"
            "      cargo xwin build --release --target x86_64-pc-windows-msvc -p er-installer\n"
            "  ER_INSTALLER_EMBED_DIR=$PWD/target/x86_64-pc-windows-msvc/release \\\n"
